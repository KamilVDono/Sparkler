...
namespace Sparkler.Editor.SharedComponentFilters
{
	public class SharedComponentFilterWindow : EditorWindow
	{
		private static readonly GUIContent s_title = new GUIContent("Shared component filter");

		#region Styles
		private static GUIStyle s_boldLabelStyle = null;

		private static GUIStyle s_BoldLabelStyle => s_boldLabelStyle ?? ( s_boldLabelStyle = new GUIStyle( EditorStyles.helpBox )
		{
			wordWrap = true,
			richText = true,
			fontSize = 16,
		...
		private void DrawGenerated()
		{
			...
			{
				EditorGUILayout.LabelField( S.Concat( "Generated code:\n" ) + ToString(), s_BoldLabelStyle );
			}
		}
	}
}