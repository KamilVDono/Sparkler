...
namespace Sparkler.Rotorz.Games.Editor
{
	/// <summary>
	...
	public sealed class ClassTypeReferencePropertyDrawer : PropertyDrawer
	{
		...
		/// <summary>
		/// Gets or sets a function that returns a collection of types that are to be excluded from
		/// drop-down. A value of <c>null</c> specifies that no types are to be excluded.
		/// </summary>
		/// <remarks>
		/// <para>
		/// This property must be set immediately before presenting a class type reference property
		/// field using <see cref="EditorGUI.PropertyField"/> or <see
		/// cref="EditorGUILayout.PropertyField"/> since the value of this property is reset to
		/// <c>null</c> each time the control is drawn.
		/// </para>
		/// <para>
		/// Since filtering makes extensive use of <see cref="ICollection{Type}.Contains"/> it is
		/// recommended to use a collection that is optimized for fast lookups such as <see
		/// cref="HashSet{Type}"/> for better performance.
		/// </para>
		/// </remarks>
		/// <example>
		...
		private static void FilterTypes( Assembly assembly, ClassTypeConstraintAttribute filter, ICollection<Type> excludedTypes, List<Type> output )
		{
			...
			{
				if ( !type.IsPublic || !( type.IsClass || type.IsValueType ) )
				{
					continue;
				}
		...
		public override float GetPropertyHeight( SerializedProperty property, GUIContent label ) => EditorStyles.popup.CalcHeight( GUIContent.none, 0 );

		public override void OnGUI( Rect position, SerializedProperty property, GUIContent label ) => DrawTypeSelectionControl( position, property.FindPropertyRelative( "classRef" ), label, attribute as ClassTypeConstraintAttribute );
	}
}