...
{
	/// <summary>
	/// xNode-specific version of <see cref="EditorGUILayout"/>
	/// </summary>
	public static class NodeEditorGUILayout
	{
		...
		/// </summary>
		public static void PropertyField( SerializedProperty property, bool includeChildren = true, params GUILayoutOption[] options ) => PropertyField( property, (GUIContent)null, includeChildren, options );
		...
		/// </summary>
		public static void PropertyField( SerializedProperty property, XNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options ) => PropertyField( property, null, port, includeChildren, options );
		...
		public static void PropertyField( SerializedProperty property, GUIContent label, XNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options )
		{
			...
			else
			{
				...
				if ( port.direction == XNode.NodePort.IO.Input )
				{
					...
					if ( NodeEditorUtilities.GetCachedAttrib( port.node.GetType(), property.name, out XNode.Node.InputAttribute inputAttribute ) )
					{
				...
				else if ( port.direction == XNode.NodePort.IO.Output )
				{
					...
					XNode.Node.ShowBackingValue showBacking = XNode.Node.ShowBackingValue.Unconnected;
					bool dynamicPortList = false;
					if ( NodeEditorUtilities.GetCachedAttrib( port.node.GetType(), property.name, out XNode.Node.OutputAttribute outputAttribute ) )
					{
				...
				if ( NodeEditorWindow.nodeTint.TryGetValue( port.node.GetType(), out Color tint ) )
				{
		...
		/// </summary>
		public static void PortField( XNode.NodePort port, params GUILayoutOption[] options ) => PortField( null, port, options );
		...
		public static bool IsDynamicPortListPort( XNode.NodePort port )
		{
			...
			if ( reorderableListCache.TryGetValue( port.node, out Dictionary<string, ReorderableList> cache ) )
			{
				if ( cache.TryGetValue( parts[0], out ReorderableList list ) )
				{
					return true;
				}
			}
		...
		public static void DynamicPortList( string fieldName, Type type, SerializedObject serializedObject, XNode.NodePort.IO io, XNode.Node.ConnectionType connectionType = XNode.Node.ConnectionType.Multiple, XNode.Node.TypeConstraint typeConstraint = XNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null )
		{
			...
			if ( reorderableListCache.TryGetValue( serializedObject.targetObject, out Dictionary<string, ReorderableList> rlc ) )
			{
		...
		public static void InstancePortList( string fieldName, Type type, SerializedObject serializedObject, XNode.NodePort.IO io, XNode.Node.ConnectionType connectionType = XNode.Node.ConnectionType.Multiple, XNode.Node.TypeConstraint typeConstraint = XNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null ) => DynamicPortList( fieldName, type, serializedObject, io, connectionType, typeConstraint, onCreation );
