...
namespace Sparkler.XNode
{
	...
	{
		/// <summary>
		/// Used during node instantiation to fix null/misconfigured graph during OnEnable/Init. Set
		/// it before instantiating a node. Will automatically be unset during OnEnable
		/// </summary>
		public static NodeGraph graphHotfix;

		/// <summary>
		/// Parent <see cref="NodeGraph"/>
		/// </summary>
		[SerializeField] public NodeGraph graph;

		/// <summary>
		/// Position on the <see cref="NodeGraph"/>
		/// </summary>
		[SerializeField] public Vector2 position;

		/// <summary>
		/// It is recommended not to modify these at hand. Instead, see <see cref="InputAttribute"/>
		/// and <see cref="OutputAttribute"/>
		/// </summary>
		...
		/// <summary>
		/// Used by <see cref="InputAttribute"/> and <see cref="OutputAttribute"/> to determine when
		/// to display the field value associated with a <see cref="NodePort"/>
		/// </summary>
		...
		public NodePort AddInstanceInput( Type type, Node.ConnectionType connectionType = Node.ConnectionType.Multiple, Node.TypeConstraint typeConstraint = TypeConstraint.None, string fieldName = null ) => AddInstanceInput( type, connectionType, typeConstraint, fieldName );

		[Obsolete( "Use AddDynamicOutput instead" )]
		public NodePort AddInstanceOutput( Type type, Node.ConnectionType connectionType = Node.ConnectionType.Multiple, Node.TypeConstraint typeConstraint = TypeConstraint.None, string fieldName = null ) => AddDynamicOutput( type, connectionType, typeConstraint, fieldName );

		[Obsolete( "Use RemoveDynamicPort instead" )]
		public void RemoveInstancePort( string fieldName ) => RemoveDynamicPort( fieldName );

		[Obsolete( "Use RemoveDynamicPort instead" )]
		public void RemoveInstancePort( NodePort port ) => RemoveDynamicPort( port );

		[Obsolete( "Use ClearDynamicPorts instead" )]
		public void ClearInstancePorts() => ClearDynamicPorts();

		[Obsolete( "Use AddDynamicPort instead" )]
		private NodePort AddInstancePort( Type type, NodePort.IO direction, Node.ConnectionType connectionType = Node.ConnectionType.Multiple, Node.TypeConstraint typeConstraint = TypeConstraint.None, string fieldName = null ) => AddDynamicPort( type, direction, connectionType, typeConstraint, fieldName );
		...
		public NodePort AddDynamicInput( Type type, Node.ConnectionType connectionType = Node.ConnectionType.Multiple, Node.TypeConstraint typeConstraint = TypeConstraint.None, string fieldName = null ) => AddDynamicPort( type, NodePort.IO.Input, connectionType, typeConstraint, fieldName );
		...
		public bool HasPort( string fieldName ) => ports.ContainsKey( fieldName );
		...
		/// <summary>
		/// Returns a value based on requested port output. Should be overridden in all derived
		/// nodes with outputs.
		/// </summary>
		...
		public class NodeTintAttribute : Attribute
		{
			...
			public NodeTintAttribute( float r, float g, float b ) => color = new Color( r, g, b );

			/// <summary>
			/// Specify a color for this node type
			/// </summary>
			/// <param name="hex">HEX color value</param>
			public NodeTintAttribute( string hex ) => ColorUtility.TryParseHtmlString( hex, out color );
			...
			public NodeTintAttribute( byte r, byte g, byte b ) => color = new Color32( r, g, b, byte.MaxValue );
		}
		...
		public class NodeWidthAttribute : Attribute
		{
			...
			public NodeWidthAttribute( int width ) => this.width = width;
		}
