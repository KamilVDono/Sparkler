...
using ComponentType = Sparkler.Editor.CodeGens.ComponentType;
...
namespace Sparkler.Editor
{
	public class CodeGenerator
	{
		...
		private static void GenerateComponent( ComponentDefinition componentDefinition )
		{
			...
			{
				componentsPath = PathExtension.SystemPath( componentDefinition.Directory );
			}
			else
			{
				componentsPath = PathExtension.SystemPath( Path.Combine( componentDefinition.Directory, "Components" ) );
			}
		...
		private static void GenerateSystems( SystemsGraph systemsGraph )
		{
			...
			var systemsPath = PathExtension.SystemPath(Path.Combine( systemsGraph.CodeGenerationPath, "Systems"));
			...
			foreach ( var system in states )
			{
				...
				template = AssignUsings( system, systemsGraph.Namespace, template );
				...
				GenerateSystemFile( systemName, systemsGraph.Namespace, systemPath, template );
			}
		}
		...
		private static string AssignUsings( SystemNode system, string namespaceName, string template )
		{
			// Namespaces
		...
		private static string AssignQueriesFieldsDefinitions( SystemNode system, string template )
		{
		...
		private static string SetupTransition( SystemNode system, SystemLambdaAction lambda, string template )
		{
		...
		private static string AssignLambdaName( SystemNode system, SystemLambdaAction lambda, string lambdaTemplate )
		{
